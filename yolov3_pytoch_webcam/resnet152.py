...
def create_modules(blocks):
    ...
    for index, x in enumerate(blocks[1:]): # 0번 block은 network info이므로
        ...
        elif (x["type"] == "maxpool"): # Upsample layer인경우
            stride = int(x["stride"])
            size = int(x["size"])
...
class Resnet152(nn.Module):
    def __init__(self, cfgfile):
        super(Resnet152, self).__init__() # Darknet이라고 이름 지음 # 클래스 초기화하기
...
model = Resnet152("cfg/yolov3_resnet152.cfg")
inp = get_test_input()
